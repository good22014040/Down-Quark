...
class CLOTHDataset(Dataset):
    def __init__(self, path): 
        ...
        self.answer_rewards = dataset["answer_rewards"]
...
class CLOTHCollator(object):
    def __init__(self, tokenizer):
        self.tokenizer = tokenizer

    def __call__(self, sequences):
        questions = [sequence['question'] for sequence in sequences]
        policy_questions = ['distractor generation: ' + question for question in questions]
        answers = [sequence['answer'] for sequence in sequences]
        answer_rewards = [sequence['answer_reward'] for sequence in sequences]

        encodings_dict = self.tokenizer(policy_questions, answers, return_tensors="pt", padding=True)
        input_ids = encodings_dict['input_ids']
        attention_mask = encodings_dict['attention_mask']

        return input_ids, attention_mask, questions, answer_rewards
...
class ConditionTrainer:
    ...
    def sample(self, step):
        ...
        sample_num = self.params.sample_num
        prompts, responses, questions, answer_rewards = [], [], [], []
        for i, batch in enumerate(tqdm(self.train_dataloader, total=len(self.train_dataloader),
                                       desc='Sampling from current policy')):
            input_ids, attention_mask, question, answer_reward = batch

            prompt = self.decode(input_ids)
            if step == 0:
                rollouts = self.ref_policy.sample(input_ids=input_ids, attention_mask=attention_mask, top_p=self.params.top_p, sample_num = sample_num)
                response = rollouts['response/text']
            else:
                input_ids, attention_mask = self.add_control_code(input_ids, attention_mask)
                rollouts = self.policy.sample(input_ids=input_ids, attention_mask=attention_mask, top_p=self.params.top_p, sample_num = sample_num)
                response = rollouts['response/text']
            
            for idx, r in enumerate(response):
                length = len(r)
                prompts.extend([prompt[idx]] * length)
                responses.extend(r)
                questions.extend([question[idx]] * length)
                answer_rewards.extend([answer_reward[idx]] * length)

        scores = self.score_model.get_reward(questions, responses, answer_rewards)
    ...
    def step(self, step_num):
        ...
        ppo_loss = ppo_loss / self.params.accimulation_steps
        ppo_loss.backward()

        if (step_num + 1) % self.params.accimulation_steps == 0:
            ...
            self.kl_ctl.update(stats['objective/kl'], self.params.batch_size * self.params.accimulation_steps, True)
            self.entropy_ctl.update(stats['objective/entropy'], self.params.batch_size * self.params.accimulation_steps, False)
    ...
    def loss(self, step, input_ids, attention_mask, labels, labels_mask):
        outputs = self.policy.forward_pass(input_ids, attention_mask, labels, labels_mask)
        ...
        masks = labels_mask.to(self.policy.device)

        with torch.no_grad():
            ref_outputs = self.ref_policy.forward_pass(input_ids[:, 1:], attention_mask[:, 1:],
                                                       labels, labels_mask)
        ...
        queries, responses = self.decode(input_ids, labels)
    ...
    def eval(self, step):
        ...
        for i, batch in enumerate(tqdm(self.val_dataloader)):
            with torch.no_grad():
                ...
                questions, responses, answer_rewards = [], [], []
                for idx, r in enumerate(response):
                    length = len(r)
                    responses.extend(r)
                    questions.extend([question[idx]] * length)
                ...
                reward = self.score_model.get_reward(questions, responses, answer_rewards)
                rewards.extend(reward)
                generations.extend(responses)

        reward_score = np.mean(rewards)
        print(f"  reward = {reward_score:+.2f}")
        self.writer.add_scalar('Evaluation/reward', reward_score, step)
...
def main():
    ...
    reward = Reward(model_path=args.reward_model_path, device=device, batch_size=args.batch_size)
    data_pool = DataPool(tree_tokens=tree_tokens, n_extra_tokens=args.n_extra_tokens)
    log.info(f'Initialization done!')

    CLOTH_collator = CLOTHCollator(tokenizer=policy.tokenizer)
    train_dataset = CLOTHDataset(path=args.dataset_train)
    train_dataloader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, drop_last=True, collate_fn=CLOTH_collator)
    log.info(f'Load train set with {len(train_dataset)} examples')

    val_dataset = CLOTHDataset(path=args.dataset_val)
    val_dataloader = DataLoader(val_dataset, batch_size=args.batch_size, shuffle=False, collate_fn=CLOTH_collator)
    log.info(f'Load val set with {len(val_dataset)} examples')

    # set up optimizer and scheduler
    optimizer = Adam(policy.model.parameters(), lr=args.lr, eps=1e-5)
    args.total_steps = ceil_div(args.total_episodes, args.batch_size * args.accimulation_steps)
    ...
    for step_num in range(args.total_steps * args.accimulation_steps):
        try:
